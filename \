...
"""
Generate empirical distribution from trajectories at a given time point.
"""
function gen_dist(trajs::Vector, t::Float64, dt::Float64)
    counts = Dict{Vector{Int}, Int}()  # STATE → COUNT (not probability!)
    
    n_valid = 0
    for traj in trajs
        # Find closest time point
        idx = argmin(abs.(traj.t .- t))
        
        # Only use if within tolerance
        if abs(traj.t[idx] - t) < dt / 2
            state = Int.(traj.u[idx])
            counts[state] = get(counts, state, 0) + 1  # INCREMENT COUNT
            n_valid += 1
        end
    end
    
    # Convert counts to probabilities
    dist = Dict{Vector{Int}, Float64}()
    for (state, count) in counts
        dist[state] = count / n_valid  # NOW convert to probability
    end
...
"""
Convert trajectories to sequence of distributions over time.
"""
function convert_to_distributions(
    trajs::Vector,
    tspan::Tuple{Float64, Float64},
    dt::Float64
)
    times = collect(tspan[1]:dt:tspan[2])
  distributions = Vector{Dict{Vector{Int}, Float64}}()
    
    for t in times
        dist = gen_dist(trajs, t, dt)
